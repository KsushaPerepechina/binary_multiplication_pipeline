...
function Pipeline(timePerIteration){
    ...
    this.multiply = function(item){
        ...
        if (currentBit == "1"){
            item.sum = item.sum.add(item.getFirst());
        }
        else
        if (currentBit == "0")
            return;
    ...
    for (var row = 0; row < size + pipelineStages; row++) {
        ...
        for (var column = 0; column < 1 + pipelineStages + 1; column++) {
            ...
            for (var pair = 0; pair < size; pair++){
                var A = stateArray[pair].first[column];
                ...
                if ((row == pair + 1) && column == 0) {
                    currentCell.innerHTML = "A: " + A.getDecimal() + '</br>' +
                        "B: " + B.getDecimal() + '</br>' +
                        "Sum: " + sum  + '</br>';
                }
                // result
                else if ((row == pair + pipelineStages) && (column == pipelineStages + 1)) {
                    A = stateArray[pair].first[column - 1];
                    var result = new BinaryNumber().toDecimal(stateArray[pair].sums[column - 1]);
                    var time = stateArray[pair].timeStamps[column - 1];
                    currentCell.innerHTML = "A: " + A.getBinaryView() + '</br>' +
                ...
                // multiplication
                else if ((row == column + pair) && (column % 2 != 0) && (column != pipelineStages + 1)){
                    currentCell.innerHTML = "A: " + A.getFormattedString() + '</br>' +
                        "B: " + B.getFormattedString() + '</br>' +
                        "Bit: " + bit + '</br>' +
                        "Sum: " + sum  + '</br>';
...
function BinaryNumber(value = 0, capacity = defaultBitCapacity){
    ...
    this.getBinaryView = function(){
        var str1 = this.getString();
        var str2 = str1.substring(defaultBitCapacity, 0);
        return str2;
    }

    this.getFormattedSumString = function(){
        var str = this.getSumString();
        return str.match(/.{1,4}/g).join("-");
    }

    this.getSumString = function() {
        var valueString = number.toString(2);
        while (valueString.length < bitCapacity) {
            while (valueString.length < defaultBitCapacity * 2) {
                valueString = "0" + valueString;
            }
            return valueString;
        }
...
function PipelineItem(firstBinaryNumber, secondBinaryNumber, initTime){
    ...
    this.sum = new BinaryNumber(0, defaultBitCapacity*2);
    ...
    this.history = {
        timeStamps: [initTime],
        first: [new BinaryNumber(firstBinaryNumber.getDecimal())],
        sums: [new BinaryNumber(0, defaultBitCapacity*2).getFormattedSumString()],
    ...
    this.writeHistory = function(currentTime){
        ...
        this.history.first.push(new BinaryNumber(this.first.getDecimal(), this.first.getCapacity()));
    ...
    this.getCurrentBit = function(){
        var length = this.second.getCapacity();
        var bitIndex = Math.floor(length - (this.getStage() / 2) - 1);
